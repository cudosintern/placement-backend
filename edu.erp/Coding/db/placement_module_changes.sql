...
-- =========================================================================
-- SECTION 5: INTEGRATION BUG FIXES
-- Safe and conditional alters to fix missing columns/tables on remote DBs.
-- =========================================================================

-- Procedure to add columns safely to plm_student_profile
DROP PROCEDURE IF EXISTS FixStudentProfileColumns;
DELIMITER //
CREATE PROCEDURE FixStudentProfileColumns()
BEGIN
    IF NOT EXISTS(
        SELECT * FROM information_schema.COLUMNS
        WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'plm_student_profile'
        AND COLUMN_NAME = 'current_cgpa'
    ) THEN
        ALTER TABLE plm_student_profile ADD COLUMN current_cgpa DECIMAL(4, 2) DEFAULT NULL;
    END IF;
    ...
    IF NOT EXISTS(
        ...
        ALTER TABLE plm_student_profile ADD COLUMN student_id INT UNSIGNED NOT NULL UNIQUE;
    END IF;
END //
DELIMITER ;
CALL FixStudentProfileColumns();
DROP PROCEDURE IF EXISTS FixStudentProfileColumns;

-- Procedure to add columns safely to plm_drive
DROP PROCEDURE IF EXISTS FixDriveColumns;
DELIMITER //
CREATE PROCEDURE FixDriveColumns()
BEGIN
    IF NOT EXISTS(
        SELECT * FROM information_schema.COLUMNS
        WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'plm_drive'
        AND COLUMN_NAME = 'job_role'
    ) THEN
        ALTER TABLE plm_drive ADD COLUMN job_role VARCHAR(150) NOT NULL;
    END IF;
END //
DELIMITER ;
CALL FixDriveColumns();
DROP PROCEDURE IF EXISTS FixDriveColumns;
...
-- Procedure to rename columns safely in plm_interview_schedule
DROP PROCEDURE IF EXISTS FixInterviewScheduleColumns;
DELIMITER //
CREATE PROCEDURE FixInterviewScheduleColumns()
BEGIN
    IF EXISTS(
        SELECT * FROM information_schema.COLUMNS
        WHERE TABLE_SCHEMA = DATABASE()
        AND TABLE_NAME = 'plm_interview_schedule'
        AND COLUMN_NAME = 'schedule_id'
    ) THEN
        ALTER TABLE plm_interview_schedule CHANGE COLUMN schedule_id id INT AUTO_INCREMENT;
    END IF;
END //
DELIMITER ;
CALL FixInterviewScheduleColumns();
DROP PROCEDURE IF EXISTS FixInterviewScheduleColumns;
